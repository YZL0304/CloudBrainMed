import torch.optim as optim  # 🔥 这里要导入 optim
...
def main():
	...
	bestModel = UNet2D(1, 1).to(DEVICE)
	bestModel.load_state_dict(torch.load(weights_file, map_location=DEVICE))
	bestModel.eval()
	# ====================== 可视化 ======================
	print("\n📊 开始生成可视化图表...")
	vision = Visualizer(save_dir=SAVE_DIR)
	vision.plot_all_curves(history)
	vision.plot_confusion(bestModel, val_loader, DEVICE)
	vision.plot_sample_pred(bestModel, val_ds, DEVICE)
